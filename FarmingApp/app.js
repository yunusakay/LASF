...
// ---------------------------------------------------------------------------
// UI HELPERS (DÜZELTİLEN YER BURASI)
// ---------------------------------------------------------------------------
...
function setBadge(id, isActive) {
    ...
    // Artık CSS hallediyor — inline style satırları kaldırıldı
}
...
async function fetchAndUpdate() {
    ...
    for (let i = 1; i <= 4; i++) {
        ...
        if (selectedCrop === 'marul') {
            growth = Math.min(100, d.chamber_humidity * 0.85); // data yerine d
            color = "#4ade80";
        } else if (selectedCrop === 'patates') {
            growth = Math.min(100, (d.mineral_k / 250) * 100); // data yerine d
            color = "#fbbf24";
        } else if (selectedCrop === 'soya') {
            growth = Math.min(100, (d.mineral_n / 200) * 100); // data yerine d
            color = "#38bdf8";
        }
...
function _logChanges(d) {
    ...
    if (d.stress_phase !== _prevStress && _prevStress !== null)
        logTerminal(`STRES DURUMU: ${_prevStress} → ${d.stress_phase}`, ...);
    _prevStress = d.stress_phase;

    if (d.devices) {
        for (const [dev, state] of Object.entries(d.devices)) { ... }
        _prevDevices = { ...d.devices };
    }
    if (d.alerts?.length > 0) d.alerts.forEach(a => logTerminal(a, 'crit'));
}

// Bu kodu fetchAndUpdate() fonksiyonunun İÇİNE ekleyin:
